...
syn match ondaSection "^\s*\zs\%(ins\|inputs\|outs\|outputs\|kouts\|params\|kins\|const\|events\|delegates\|tasks\|buffers\|init\|block\|sample\|graph\)\ze\%(\s*<\|\s\+\d\+\|\s*:\|\s*{\|\s\+[A-Za-z_(]\)"
...
syn match ondaDefDecl "^\s*\zs\%(def\|event\|delegate\|task\)\ze\>" nextgroup=ondaFunctionName skipwhite
...
syn keyword ondaKeyword if elif else for in while loop break continue return await yield assert use as pub private pin config when print
